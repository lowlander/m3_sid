...
static void c64_set_mem8(uint16_t address, uint8_t val)
{
	HW_set_8bit_reg((addr_t)CORESDR_AHB_C1_0 + (address), val);
}

static uint8_t c64_get_mem8(uint16_t address)
{
	return (HW_get_8bit_reg((addr_t)CORESDR_AHB_C1_0 + (address)) & 0xFF);
}

uint8_t c64_getmem(uint16_t addr)
{
    return c64_get_mem8(addr);
}
